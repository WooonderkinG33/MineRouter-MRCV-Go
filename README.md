...
Формат `.mrcv` — стабильный контракт: magic, смещения полей, параметры
Argon2id, шифр и AAD остаются неизменными между реализациями.

- Изменения, ломающие формат или криптографию, допускаются **только в новой
  мажорной версии** (semver) — обе реализации (JS и Go) обновляются вместе.
- Изменения API-обёртки (методы, конфигурация, ошибки) мажорную версию не
  трогают — они не влияют на байты файла на диске.
